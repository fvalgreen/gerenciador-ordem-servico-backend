...
export const validarToken = (handler: NextApiHandler) => (req: NextApiRequest, res: NextApiResponse<ResponseDefault>) => {
  try {
    const { JWT_KEY } = process.env;

    if(!JWT_KEY){
      return res.status(500).json({erro: 'ENV da chave JWT não foi informada'})
    };
